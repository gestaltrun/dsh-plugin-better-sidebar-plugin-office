#!/usr/bin/env node
/** Build, verify, and optionally publish the Gestaltrun Office viewer package. */
...
export const PACKAGE_NAME = '@gestaltrun/dsh-sidebar-office'
export const REPOSITORY = 'gestaltrun/dsh-plugin-better-sidebar-plugin-office'
export const SIDEBAR_NAME = '@gestaltrun/dsh-better-sidebar'
export const SIDEBAR_VERSION = '0.19.1-gestaltrun.0'

/** Validate the immutable package identity and release policy. */
export function assertPackage(manifest, packed = false) {
  ...
  if (manifest.license !== 'AGPL-3.0') throw new Error('The upstream AGPL-3.0 license must be retained')
...
export function assertSidebarTarball(path) {
  ...
  if (manifest.name !== SIDEBAR_NAME || manifest.version !== SIDEBAR_VERSION) {
    throw new Error(`Expected ${SIDEBAR_NAME}@${SIDEBAR_VERSION}`)
  }
}
...
function installBuildGraph(sidebar) {
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'dsh-sidebar-office-'))
  try {
...
function pack(manifest, inputs) {
  ...
  for (const required of ['package/lib/index.js', 'package/lib/client.js', 'package/src/client/index.tsx', 'package/cordis.patch.yml', 'package/LICENSE']) {
...
/** Require a release tag or an opted-in manual workflow before publication. */
export function assertPublishContext(manifest, env) {
  ...
  if (env.GITHUB_EVENT_NAME !== 'workflow_dispatch' || env.PUBLISH_PACKAGE !== 'true') {
    throw new Error('Publishing requires a release or opted-in manual workflow')
  }
}
